const initialize = (self: any /*ServiceWorkerGlobalScope*/, settings: Record<string, any>): void => {
  ...
  const requestIsNavigation = (request: Request): boolean => {
    return (request.mode === "navigate" || (request.method === "GET" && request.headers.get("accept").includes("text/html")));
  };

  const requestIsImage = (request: Request): boolean => {
    return (request.method === "GET" && request.destination === "image");
  };

  const requestIsFavicon = (request: Request): boolean => {
  ...
  const handleDefault = (request: Request, response?: Response): Promise<Response> => {
  ...
  const responseFromCache = (request: Request): Promise<Response> => {
  ...
  const fetchResponse = (request: Request): Promise<Response> => {
  ...
  self.addEventListener("install", (event) => {
    ...
    if(settings.defaultHTML) {
      resourcesList.push(settings.defaultHTML);
    }

    if(settings.defaultImage) {
      resourcesList.push(settings.defaultImage);
    }

    if(settings.defaultFavicon) {
      resourcesList.push(settings.defaultFavicon);
    }

    event.waitUntil(addToCache(resourcesList).then(() => self.skipWaiting()));
  });
